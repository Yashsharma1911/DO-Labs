...
# -------------------------------------------------------------------
locals {
  # No longer need base64 encoding since we're downloading the file
}

# -------------------------------------------------------------------
# Web droplets with custom UI
...
# user_data bootstraps nginx, Docker, and deploys our custom web UI.
# -------------------------------------------------------------------
resource "digitalocean_droplet" "web" {
  ...
  # resolve the key's ID. Empty list if no key specified.
  ssh_keys = var.ssh_key != null ? [data.digitalocean_ssh_key.default[0].id] : []

  # user_data is executed at first boot to install nginx, Docker, and deploy our UI
  user_data = <<-EOF
#!/bin/bash
apt-get update
...
sed -i "s|<div class=\"region-name\" id=\"regionName\">Loading...</div>|<div class=\"region-name\" id=\"regionName\">${each.value.region}</div>|g" /var/www/html/index.html

# Method 2: Create a backup and use a more reliable approach
cp /var/www/html/index.html /var/www/html/index.html.backup

# Method 3: Use a simple text replacement that's more reliable
REGION_NAME="${each.value.region}"
sed -i "s/Loading\.\.\./$REGION_NAME/g" /var/www/html/index.html

# Method 4: Add region info to the page title as well
sed -i "s|<title>DigitalOcean Deploy 2025</title>|<title>DigitalOcean Deploy 2025 - Region: $REGION_NAME</title>|g" /var/www/html/index.html

# Log the region for debugging
echo "Deployed in region: $REGION_NAME" > /var/www/html/region.txt
...
server {
    ...
    location /repo {
        return 302 ${var.github_repo_url};
    }

    # Health check endpoint for load balancers
    location /health {
        access_log off;
        return 200 "healthy - region: ${each.value.region} - droplet: ${each.value.droplet}\n";
...
cat > /var/www/html/status.json << STATUS_JSON
{
  "region": "${each.value.region}",
  "droplet": "${each.value.droplet}",
  "name": "${var.name_prefix}-${each.value.region}-${each.value.droplet}",
  "deployed_at": "$(date -Iseconds)",
  "architecture_url": "${var.architecture_image_url}",
